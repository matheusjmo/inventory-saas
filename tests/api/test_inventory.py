...
async def _create_product_and_item(client: AsyncClient, sku: str = "SHOE-001", qty: int = 0):
...
async def test_register_inventory_item(client: AsyncClient):
    ...
    response = await client.post("/inventory", json={"sku": "SHOE-001", "initial_quantity": 0})
...
async def test_register_inventory_item_product_not_found(client: AsyncClient):
    response = await client.post("/inventory", json={"sku": "NOTEXIST", "initial_quantity": 0})
...
async def test_apply_movement_in(client: AsyncClient):
    await _create_product_and_item(client, qty=10)
    response = await client.post("/inventory/movements", json={
        "movement_type": "in",
        "lines": [{"sku": "SHOE-001", "quantity": 5}],
    })
    assert response.status_code == 201
...
async def test_apply_movement_out_insufficient_stock(client: AsyncClient):
    ...
    response = await client.post("/inventory/movements", json={
        "movement_type": "out",
        "lines": [{"sku": "SHOE-001", "quantity": 10}],
    })
    assert response.status_code == 422
...
async def test_get_movement_history(client: AsyncClient):
    await _create_product_and_item(client, qty=10)
    await client.post("/inventory/movements", json={
        "movement_type": "in",
        "lines": [{"sku": "SHOE-001", "quantity": 5}],
    })
